...
public class StateTable {
	...
	public State nextState(ECXEvent event) {
		return currentState = nextState(currentState, event);
	}

	public State nextState(List<ECXEvent> events) {
		return currentState = nextState(currentState, events);
	}

	public State nextState(State from, ECXEvent event) {
		return (isTransitionExists(from, event) ? getTo(from, event) : from);
	}
	...
	public boolean isTransitionExists(State from, ECXEvent event) {
		return (getTo(from, event) != null);
	}

	public boolean isTransitionExists(Transition trans) {
		return (getTo(trans.getFrom(), trans.getEvent()) != null);
	}

	private State getTo(State from, ECXEvent event) {
		Transition T = getTransition(from, event);
		return (T != null) ? T.getTo() : null;
	}

	public void addTransitions(List<Transition> transList) {
		for (Transition trans : transList)
			addTransition(trans);
	}

	public void addTransition(Transition trans) { // override any existing matching transitions.
		if (isTransitionExists(trans))
			removeTransition(trans);
		transitions.add(trans);
	}

	public void removeTransition(Transition trans) {
		Transition T = getTransition(trans.getFrom(), trans.getEvent());
		if (T != null)
	...
	public StateTable setName(String name) {
		this.name = name;
		return this;
	}
}