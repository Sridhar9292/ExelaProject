...
@Slf4j	
@RestController
@RequestMapping("/invalid-message-reports")
public class InvalidMessageReportsController {
    static int BUFFER_SIZE = 4096;
    @Value("${reportPath.invalidMessage:}")
	...
	@PreAuthorize("hasAnyAuthority('_reports:monthlyReport:view','_reports:monthlyReport:manage')")
	@Operation(summary= "List of Invalid Message Reports")
    @GetMapping
    public List<SummaryReport> invalidReport() throws NotFoundException {

        List<SummaryReport> summaryFiles = new ArrayList<>();
        try {
            SummaryReport summaryReport = null;
            System.out.println("path===>>" + BASE_REPORT_PATH);
            File dir = new File(BASE_REPORT_PATH);
            FileFilter fileFilter = new RegexFileFilter("^([a-zA-Z]{20,20}\\d{8}.txt)$");
	...
	@PreAuthorize("hasAnyAuthority('_reports:monthlyReport:view','_reports:monthlyReport:manage')")
	@Operation(summary= "Download InvalidMessageReports ")
    @GetMapping("/{filename}")
    public ResponseEntity<?> invalidReportDownload(@PathVariable("filename") String filename) throws NotFoundException {
        try {
	...
	@PreAuthorize("hasAnyAuthority('_reports:invalidMessageReport:manage','_reports:invalidMessageReport:generateReport')")
	@Operation(summary= "generated InvalidMessageReports date format yyyy-MM-dd")
	@GetMapping("/{boundmsg}/{fromDate}/{toDate}")
	public  ResponseEntity<?>  generateInvalidMessageReport(@PathVariable("boundmsg") String boundmsg,@PathVariable("fromDate") String fromDate,@PathVariable("toDate") String toDate)
		...
		try {
			...
			// Create and send the message
			  ...
			  TextMessage acgMsg = session.createTextMessage();
	            acgMsg.setStringProperty("eventName","ACGInvalidMessageExport");
